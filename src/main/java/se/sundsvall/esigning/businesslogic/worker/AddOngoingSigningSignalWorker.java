...
public class AddOngoingSigningSignalWorker extends AbstractWorker {
	...
	AddOngoingSigningSignalWorker(EngineClient engineClient, FailureHandler failureHandler, Gson gson, DocumentClient documentClient) {
	...
	public void executeBusinessLogic(ExternalTask externalTask, ExternalTaskService externalTaskService) {
		...
		try {
			...
			failureHandler.handleException(externalTaskService, externalTask, "%s occurred for document %s with registration number %s when updating status (%s).".formatted(
